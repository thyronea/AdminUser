<nav class="navbar navbar-expand-lg bg-body sticky-top" id="navbar">
  <div class="container">
    <a class="navbar-brand" href="/index.php">DLi</a>
    <a class="nav-item" href="index.php">Scrollspy 1</a>
    <a class="nav-item" href="#scrollspy2">Scrollspy 2</a>
    <a class="nav-item" href="#scrollspy3">Scrollspy 3</a>
    <a class="nav-item" href="#scrollspy4" >Scrollspy 4</a>
    <a class="nav-item" href="#scrollspy5" >Scrollspy 5</a>
    <div class="collapse navbar-collapse" id="navbarSupportedContent">
      <ul class="navbar-nav ms-auto mb-2 mb-lg-0">
        <li class="nav-item dropdown">
          <a class="nav-link active dropdown-toggle" href="#" role="button" data-bs-toggle="dropdown" aria-expanded="false">
            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" class="bi bi-person" viewBox="0 0 16 16">
              <path d="M8 8a3 3 0 1 0 0-6 3 3 0 0 0 0 6Zm2-3a2 2 0 1 1-4 0 2 2 0 0 1 4 0Zm4 8c0 1-1 1-1 1H3s-1 0-1-1 1-4 6-4 6 3 6 4Zm-1-.004c-.001-.246-.154-.986-.832-1.664C11.516 10.68 10.289 10 8 10c-2.29 0-3.516.68-4.168 1.332-.678.678-.83 1.418-.832 1.664h10Z"/>
            </svg>
          </a>
          <div class="dropstart">
            <ul class="dropdown-menu">
              <li><a class="dropdown-item" href="/public/page/access/sign-in.php"><small>Login</small></a></li>
              <li><a class="dropdown-item" href="/public/page/access/register.php"><small>Register</small></a></li>
            </ul>
          </div>
        </li>
      </ul>
    </div>
  </div>
</nav>
