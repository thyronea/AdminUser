<?php
session_start();
include('../../../../../access/dbcon.php'); // database connection
...
if(isset($_POST['register_btn']))
{
  ...
  http://localhost:8000/private/security/new-user-email-verification.php?token=$token
  ...
  else
  {
    ...
    $stmt = $con->prepare($sql);
    $stmt->bind_param("sssssss", $newID, $engineID, $groupID, $encrypted_fname, $encrypted_lname, $encrypted_email, $encrypted_role);
    $stmt->execute();
    ...
    $link = mysqli_real_escape_string($con, "page/admin/index.php"); // Link to user's profile
